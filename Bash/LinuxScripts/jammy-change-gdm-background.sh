...
if ! [[ "$distro" =~ (jammy) ]]; then
    echo 'Sorry, this script only works with Jammy distros.'
    exit 1
fi
...
<gresources>
    ...
    echo 'Usage: sudo ./change-gdm-background-jammy /path/to/image.*g'
    echo 'Usage: sudo ./change-gdm-background-jammy \#yourhexcode'
